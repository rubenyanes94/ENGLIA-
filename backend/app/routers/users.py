...
from app.schemas.descriptor import DescriptorMasteryOut, DescriptorMasterySummaryOut, ExitCriterionOut, LevelExitGateOut
    ...
    if exit_tasks_cfg:
        ...
        for task_cfg in exit_tasks_cfg:
            ...
            times_completed = await descriptor_evidence_repository.count_distinct_successful_sessions(
                db, current_user.id, context=task_id
            )
            tasks_detail.append(
                {
                    "task_id": task_id,
