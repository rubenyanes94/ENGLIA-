...
export default function ModuleDetailPage() {
  ...
  return (
    <div className="space-y-8">
      <Link to="/classroom" className="inline-flex items-center gap-2 text-sm text-slate-400 hover:text-slate-600">
      ...
      <header className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm lg:p-8">
        <div className="flex flex-col gap-5 lg:flex-row lg:items-start lg:justify-between">
          <div>
            ...
            <h1 className="mt-1 text-3xl font-extrabold text-slate-900 lg:text-4xl">{module.title}</h1>
            <p className="mt-1 text-lg text-slate-500">{module.title_es}</p>
          ...
          <button
            ...
            className="flex shrink-0 items-center justify-center gap-2 rounded-full bg-blue-600 px-6 py-3 text-sm font-semibold text-white transition active:scale-[0.98] hover:bg-blue-500 disabled:cursor-not-allowed disabled:bg-slate-200 disabled:text-slate-400"
          >
      ...
      {/* Contenido curricular a la izquierda, tareas (lo accionable) fijas
          a la derecha en escritorio: son el punto de entrada al tutor, no
          deberían quedar al final de un scroll largo. */}
              ...
              {module.lexis.sets && module.lexis.sets.length > 0 && (
                <div className="mb-5">
                  <p className="mb-2 text-sm text-slate-400">
                    Campos léxicos ({module.lexis.target_items ?? "?"} ítems objetivo)
                  </p>
                ...
                <div>
                  ...
                  <ul className="grid gap-2 sm:grid-cols-2">
                    {module.lexis.chunks.map((chunk) => (
                      <li key={chunk} className="rounded-xl bg-blue-50 px-3 py-2 text-sm text-blue-700">
                        "{chunk}"
                      </li>
                    ))}
                  </ul>
          ...
          {module.pronunciation.focus && (
            <Section title="Pronunciación" icon={faVolumeHigh}>
              ...
              {module.pronunciation.l1_alerts && module.pronunciation.l1_alerts.length > 0 && (
                <ul className="mt-3 list-inside list-disc space-y-1 text-sm text-amber-600">
            ...
            <Section title="Interferencia L1 (hispanohablantes)" icon={faTriangleExclamation}>
              <div className="grid gap-3 xl:grid-cols-2">
                {module.l1_interference.map((item) => (
                  <div key={item.error} className="rounded-xl border border-slate-200 bg-slate-50 p-4">
                    ...
                    <p className="mt-2 text-xs text-slate-400">Origen: {item.origin}</p>
            ...
            <Section title="Lecciones" icon={faBookOpen}>
              <ul className="space-y-2">
                {module.lessons.map((lesson) => (
                  <li key={lesson.id} className="rounded-xl bg-slate-100 px-4 py-2.5 text-sm text-slate-600">
            ...
            {module.tasks.map((task) => (
              <div key={task.id} className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
                ...
                {task.success_criteria && (
                  <p className="mt-2 text-xs text-slate-400">Se logra si: {task.success_criteria}</p>
                )}
                <button
                  onClick={() => navigate(`/chat?module=${module.id}&task=${task.id}`)}
                  className="mt-4 flex w-full items-center justify-center gap-2 rounded-full bg-blue-100 px-4 py-2.5 text-xs font-semibold text-blue-700 transition active:scale-[0.98] hover:bg-blue-200"
                >
